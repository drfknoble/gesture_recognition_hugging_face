...
def classify(frame, feature_extractor, model, device):
    '''
    Given an frame, the function classifies the ROI, and returns the predicted class
    @param frame - input frame
    @param feature_extractor - HuggingFace feature extractor for model
...
arg_parser.add_argument('-m', '--model', default='D:/saved_models/vit-base-patch16-224-in21k-finetuned-gestures/', type=str, help='Model')
    ...
    while key != 27:
        ...
        if results.multi_hand_landmarks:
            ...
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                result = classify(hand_rs, feature_extractor, model, device)
                ...
                cv.putText(drawing, f"{result['elapsed'] * 1000:.2f} ms", elapsed_pt, cv.FONT_HERSHEY_PLAIN, 2.0, (0,0,255), 2)
